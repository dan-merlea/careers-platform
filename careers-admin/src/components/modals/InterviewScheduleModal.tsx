...
const InterviewScheduleModal: React.FC<InterviewScheduleModalProps> = ({
  ...
  // Helper function to check if a time slot can fit the interview duration
  const canFitInterviewDuration = (selectedDate: string, selectedTime: string): boolean => {
    if (availableTimeSlots.length === 0) return true; // No restrictions if no slots provided
    if (!selectedDate || !selectedTime) return false;

    // Parse the selected date and time
    const [hours, minutes] = selectedTime.split(':').map(Number);
    const interviewStartMinutes = hours * 60 + minutes;
    const interviewEndMinutes = interviewStartMinutes + interviewDuration;
    ...
    // Check if the interview can fit within consecutive available slots
    for (let i = 0; i < sortedSlots.length; i++) {
      const [slotStartH, slotStartM] = sortedSlots[i].startTime.split(':').map(Number);
      const [slotEndH, slotEndM] = sortedSlots[i].endTime.split(':').map(Number);
      const slotStartMinutes = slotStartH * 60 + slotStartM;
      const slotEndMinutes = slotEndH * 60 + slotEndM;

      // Check if interview can start in this slot
      if (interviewStartMinutes >= slotStartMinutes && interviewStartMinutes < slotEndMinutes) {
        // Find consecutive slots that cover the interview duration
        let currentEndMinutes = slotEndMinutes;
        
        // Check consecutive slots
        for (let j = i + 1; j < sortedSlots.length; j++) {
          const [nextStartH, nextStartM] = sortedSlots[j].startTime.split(':').map(Number);
          const [nextEndH, nextEndM] = sortedSlots[j].endTime.split(':').map(Number);
          const nextStartMinutes = nextStartH * 60 + nextStartM;
          const nextEndMinutes = nextEndH * 60 + nextEndM;
          ...
          currentEndMinutes = Math.max(currentEndMinutes, nextEndMinutes);
        }

        // Check if we have enough continuous time
        if (interviewEndMinutes <= currentEndMinutes) {
          return true;
        }
      }
    }
  ...
  // Get all time options (30-minute intervals from 7:00 to 20:00)
  const getAllTimeOptions = (): string[] => {
    const times: string[] = [];
  ...
  const getDatesWithAvailability = (): Set<string> => {
  ...
  const handleSubmit = async () => {
    ...
    if (!isEditMode) {
      ...
      // Warn if the selected time doesn't fit within candidate availability
      if (availableTimeSlots.length > 0 && !canFitInterviewDuration(date, time)) {
        const confirmed = window.confirm(
          `⚠️ Warning: The selected time may not fit within the candidate's ${interviewDuration}-minute availability window.\n\nDo you want to schedule anyway? You may need to coordinate with the candidate separately.`
        );
        if (!confirmed) {
          return;
        }
      ...
      <div className="flex items-center justify-center min-h-screen">
        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 p-6">
          ...
          <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label htmlFor="time" className="block text-sm font-medium text-gray-700 mb-1">
                      Time * {availableTimeSlots.length > 0 && <span className="text-xs text-gray-500">({interviewDuration} min, <span className="text-green-600">🟢 = fits availability</span>)</span>}
                    </label>
                    {availableTimeSlots.length > 0 ? (
                      <Select
                        value={time}
                        onChange={(val) => setTime(val || '')}
                        options={getAllTimeOptions().map((timeOption) => ({
                          label: timeOption,
                          value: timeOption,
                          highlighted: date ? canFitInterviewDuration(date, timeOption) : false,
                          highlightColor: '#dcfce7'
                        }))}
                        placeholder="Select a time"
                        disabled={isSubmitting || !date}
                        ariaLabel="Interview time"
                      />
                    ) : (
                      <Input
                        type="time"
            ...
            <div className="bg-blue-50 border border-blue-200 text-blue-700 px-4 py-3 mt-4 rounded-md">
