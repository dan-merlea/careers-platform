...
export default function TimeSlotsPage() {
  ...
  // Generate time slots for a day (7 AM to 8 PM, 30-minute intervals)
  ...
  // Fetch applicant data and interview info
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(`/api/timeslots/${applicationId}/interview/${interviewId}`);
        if (!response.ok) {
        ...
        // Load existing time slots for this interview
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
          ...
          {applicantData && (
            <div className="text-sm text-gray-600 space-y-1">
              ...
              <p><span className="font-medium">Application Stage:</span> {applicantData.status}</p>
            </div>
          )}
        </div>
        ...
        {applicantData?.interview && (
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 mb-6">
            <h2 className="text-lg font-semibold text-blue-900 mb-3">Interview Details</h2>
            <div className="space-y-2 text-sm text-blue-800">
              <p><span className="font-medium">Title:</span> {applicantData.interview.title}</p>
              {applicantData.interview.description && (
                <p><span className="font-medium">Description:</span> {applicantData.interview.description}</p>
              )}
              {applicantData.interview.stage && (
                <p><span className="font-medium">Stage:</span> {applicantData.interview.stage}</p>
              )}
              {applicantData.interview.location && (
                <p><span className="font-medium">Location:</span> {applicantData.interview.location}</p>
              )}
              {applicantData.interview.durationMinutes && (
                <p><span className="font-medium">Duration:</span> {applicantData.interview.durationMinutes} minutes</p>
              )}
            </div>
          </div>
        )}
        ...
        {isEditMode && (
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
            <p className="text-sm text-yellow-800">
              <strong>Instructions:</strong> Click or drag to select time slots when you&apos;re available for this interview. 
