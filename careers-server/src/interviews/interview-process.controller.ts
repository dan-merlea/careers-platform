...
export class InterviewProcessController {
  ...
  async findAll(
    ...
    const processes = await this.interviewProcessService.findAll(req.user.companyId);
  ...
  async findByJobRole(
    ...
    const processes = await this.interviewProcessService.findByJobRole(jobRoleId);
    return processes.map(process => this.mapToResponseDto(process));
  }
  ...
  private mapToResponseDto(process: InterviewProcessDocument): InterviewProcessResponseDto {
